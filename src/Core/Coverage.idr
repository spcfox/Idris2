...
altMatch (DelayCase {}) (DelayCase {}) = True
...
buildArgs fc defs known not ps cs@(Case idx el ty altsIn)
  ...
  where
    mkCon : ConTag -> Nat -> ClosedTerm
    mkCon (DConTag n t) ar = Ref fc (DataCon t ar) n
    mkCon (TConTag n) ar = Ref fc (TyCon ar) n
