...
mutual
  ...
  lcheck {vars} rig erase env (Local {name} fc x idx prf)
      = let b = getBinder prf env
            ...
            do log "quantity" 15 "lcheck Local"
               ...
               pure (Local fc x idx prf, !(nf env ty), used rig)
    where
      ...
      used r = if isLinear r then Usage.single (MkVar prf) else Usage.empty
  ...
  lcheck rig erase env (Ref fc nt fn)
      = do logC "quantity" 15 $ do pure "lcheck Ref \{show (nt)} \{show !(toFullNames fn)}"
           ...
           pure (Ref fc nt fn, !(nf env (embed ty)), Usage.empty)
           ...
           if expand
              then expandMeta rig erase env n idx (definition gdef) (map snd args)
                             ...
                             = case definition gdef of
                                    ...
                                    _ => type gdef
                      nty <- Value.expand !(nf env (embed ty))
  ...
  lcheck rig_in erase env (Bind fc nm b sc)
      = do log "quantity" 15 "lcheck Bind"
           ...
           discharge env fc nm b' bt sc' sct (usedb ++ doneScope usedsc)
    where
      rig : RigCount
      rig = case b of
                 Pi _ _ _ _ =>
  ...
  lcheck rig erase env (App fc f c a)
      = do logC "quantity" 15 $ do pure "lcheck App \{show !(toFullNames f)} \{show !(toFullNames a)}"
           ...
           defs <- get Ctxt
           fty <- expand gfty
           logNF "quantity" 15 "lcheck App fty" env fty
           case fty of
                VBind _ _ (Pi _ rigf _ ty) scdone =>
                   ...
                   do let checkRig = rigf |*| rig
                      logTerm "quantity" 15 "!!! lcheck App VBind" a
                      ...
                      when (debugElabCheck opts) $ do
                        aty <- expand gaty
                        when (not !(convert env aty !(expand ty))) $
                           do ty' <- quote env ty
                              aty' <- quote env aty
                ...
                -- VPrimOp _ fn args => do
                --   log "quantity" 15 $ "Linearity checking failed on PrimOp"
                --   pure (App fc f c a, gErased fc, Usage.empty)
                _ => do
                  logNF "quantity" 15 "Linearity checking failed on default" env fty
                  needFunctionType f' gfty
    where
      needFunctionType : Term vars -> Glued vars -> Core _
      needFunctionType f gfty =
        do tfty <- quote env gfty
  ...
  lcheck rig erase env (As fc s as pat)
      = do log "quantity" 15 "lcheck As 1"
           (as', _, _) <- lcheck rig erase env as
           log "quantity" 15 "!!! lcheck As 2"
  ...
  lcheck rig erase env (TDelay fc r ty val)
      = do log "quantity" 15 "!!! lcheck Delay 1"
           (ty', _, _) <- lcheck erased erase env ty
           log "quantity" 15 "!!! lcheck Delay 2"
           (val', gty, u) <- lcheck rig erase env val
           ty <- quote env gty
           pure (TDelay fc r ty' val', !(nf env (TDelayed fc r ty)), u)
  ...
  lcheck rig erase env (TType fc u)
      ...
      = do log "quantity" 15 "lcheck TType"
           pure (TType fc u, gType fc (MN "top" 0), Usage.empty)
  lcheck _ _ _ (Case _ _ _ _ _ _) = ?Case
  lcheck _ _ _ (PrimOp _ _ _) = ?PrimOp
  lcheck _ _ _ (Unmatched _ _) = ?Unmatched
  ...
  lcheckBinder rig erase env (Lam fc c x ty)
      = do log "quantity" 15 "!!! lcheck Lam"
           (tyv, tyt, _) <- lcheck erased erase env ty
           pure (Lam fc c x tyv, tyt, Usage.empty)
  lcheckBinder rig erase env (Let fc rigc val ty)
      = do log "quantity" 15 "!!! lcheck Let 1"
           (tyv, tyt, _) <- lcheck erased erase env ty
           log "quantity" 15 "!!! lcheck Let 2"
           (valv, valt, vs) <- lcheck (rig |*| rigc) erase env val
           pure (Let fc rigc valv tyv, tyt, vs)
  lcheckBinder rig erase env (Pi fc c x ty)
      = do log "quantity" 15 "!!! lcheck Pi"
           (tyv, tyt, _) <- lcheck (rig |*| c) erase env ty
           pure (Pi fc c x tyv, tyt, Usage.empty)
  lcheckBinder rig erase env (PVar fc c p ty)
      = do log "quantity" 15 "!!! lcheck PVar"
           (tyv, tyt, _) <- lcheck erased erase env ty
           pure (PVar fc c p tyv, tyt, Usage.empty)
  lcheckBinder rig erase env (PLet fc rigc val ty)
      = do log "quantity" 15 "!!! lcheck PLet 1"
           (tyv, tyt, _) <- lcheck erased erase env ty
           log "quantity" 15 "!!! lcheck PLet 2"
           (valv, valt, vs) <- lcheck (rig |*| rigc) erase env val
           pure (PLet fc rigc valv tyv, tyt, vs)
  lcheckBinder rig erase env (PVTy fc c ty)
      = do log "quantity" 15 "!!! lcheck PVty"
           (tyv, tyt, _) <- lcheck erased erase env ty
           pure (PVTy fc c tyv, tyt, Usage.empty)
  ...
  discharge : {vars : _} ->
              ...
              Core (Term vars, Glued vars, Usage vars)
  discharge env fc nm b@(Lam fc' c x ty) gbindty scope gscopety used
       = do scty <- quote (env :< b) gscopety
            pure (Bind fc nm (Lam fc' c x ty) scope,
                  !(nf env (Bind fc nm (Pi fc' c x ty) scty)), used)
  discharge env fc nm b@(Let fc' c val ty) gbindty scope gscopety used
       = do scty <- quote (env :< b) gscopety
            pure (Bind fc nm (Let fc' c val ty) scope,
                  !(nf env (Bind fc nm (Let fc' c val ty) scty)), used)
  discharge env fc nm (Pi fc' c x ty) gbindty scope gscopety used
       = pure (Bind fc nm (Pi fc' c x ty) scope, gbindty, used)
  discharge env fc nm b@(PVar fc' c p ty) gbindty scope gscopety used
       = do scty <- quote (env :< b) gscopety
            pure (Bind fc nm (PVar fc' c p ty) scope,
                  !(nf env (Bind fc nm (PVTy fc' c ty) scty)), used)
  discharge env fc nm b@(PLet fc' c val ty) gbindty scope gscopety used
       = do scty <- quote (env :< b) gscopety
            pure (Bind fc nm (PLet fc' c val ty) scope,
                  !(nf env (Bind fc nm (PLet fc' c val ty) scty)), used)
  discharge env fc nm (PVTy fc' c ty) gbindty scope gscopety used
  ...
  getArgUsage topfc rig ty pats
    ...
    where
      ...
      getPUsage : ClosedTerm -> Clause -> Core (List (Name, ArgUsage))
  ...
  lcheckMeta rig erase env fc n idx
             ((c, arg) :: args) chk (VBind _ _ (Pi _ rigf _ ty) sc)
      = do let checkRig = rigf |*| rig
           log "quantity" 15 "!!! lcheckMeta Pi"
           (arg', gargTy, aused) <- lcheck checkRig erase env arg
           defs <- get Ctxt
           sc' <- expand !(sc (nf env arg'))
  ...
  lcheckMeta rig erase env fc n idx [] chk nty
      = do defs <- get Ctxt
           pure (Meta fc n idx (reverse chk), asGlued nty, Usage.empty)
