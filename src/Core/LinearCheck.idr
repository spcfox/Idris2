...
parameters {auto c : Ref Ctxt Defs}
  ...
  updateHoleType : {vars : _} ->
                   ...
                   Core (Term vs)
  updateHoleType useVar var zs (Bind bfc nm (Pi fc' c e ty) sc) (Just (Local _ _ v _) :: as)
      ...
      = if varIdx var == v
           then do scty <- updateHoleType False var zs sc as
                ...
                then do scty <- updateHoleType useVar var zs sc as
                        ...
                        pure (Bind bfc nm (Pi fc' c e ty) scty)
  updateHoleType useVar var zs (Bind bfc nm b@(Pi fc' c e ty) sc) (_ :: as)
      = do scty <- updateHoleType useVar var zs sc as
           pure (Bind bfc nm b scty)
  updateHoleType useVar var zs ty as
      = pure ty

  updateHoleApp : {vars : _} ->
                  (useVar : Bool) -> Var vars -> List (Var vars) ->
                  HoleApp vars -> Core ()
  updateHoleApp useVar v zs hole
      = do defs <- get Ctxt
           ...
           case definition gdef of
                Hole _ _ =>
                  do let ty = type gdef
                     ty' <- updateHoleType useVar v zs ty (scope hole)
                     updateTy i ty'
  ...
  updateHoleUsage : {vars : _} ->
                    ...
                    List (Var vars) -> -- erased variables in environment
  ...
  checkEnvUsage {done} {vars = xs :< nm} fc rig (bs :< b) usage tm
      = do let pos = localPrf {later=done} {vars=xs} {n=nm}
           ...
           -- checking binders
           holeFound <- if isLinear (multiplicity b)
                           then updateHoleUsage pos [] [<] usage
                           else pure False
           let used = if isLinear (rigMult (multiplicity b) rig) &&
                         holeFound && used_in == 0
                         then 1
