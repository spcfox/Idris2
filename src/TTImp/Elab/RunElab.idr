...
elabScript rig fc nest env script@(NDCon nfc nm t ar args) exp
  ...
  where
    ...
    scriptRet tm
        = do defs <- get Ctxt
             nfOpts withAllCBV defs env !(reflect fc defs False env tm)
    ...
    elabCon defs "Map" [_,_,fm,act]
        ...
        = do act <- elabScript rig fc nest env !(evalClosure defs act) exp
             ...
             applyToStack defs withHoles env fm [(getLoc act, MkNFClosure withAllCBV env act)]
    elabCon defs "Ap" [_,_,actF,actX]
        ...
        = do actF <- elabScript rig fc nest env !(evalClosure defs actF) exp
             ...
             applyToStack defs withHoles env actF [(getLoc actX, MkNFClosure withAllCBV env actX)]
    elabCon defs "Bind" [_,_,act,k]
        ...
        = do act <- elabScript rig fc nest env
             ...
             r <- applyToStack defs withAllCBV env k [(getLoc act, MkNFClosure withAllCBV env act)]
    ...
    elabCon defs "Lambda" [x, _, scope]
        = do empty <- clearDefs defs
             ...
             n <- genVarName "x"
             sc' <- sc defs (toClosure withAllCBV env (Ref bfc Bound n))
    ...
    elabCon defs "Goal" []
        = do let Just gty = exp
                 | Nothing => nfOpts withAllCBV defs env
...
checkRunElab rig elabinfo nest env fc reqExt script exp
    = do expected <- mkExpected exp
         ...
         nfstm <- nfOpts withAllCBV defs env stm
