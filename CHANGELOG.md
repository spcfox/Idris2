...
This CHANGELOG describes the history of already-released versions. Please see
[CHANGELOG_NEXT](./CHANGELOG_NEXT.md) for changes merged into the main branch
...
- Added `fromRight` and `fromLeft` for extracting values out of `Either`,
  equivalent to `fromJust` for `Just`.
...
- Refactored `Uninhabited` implementation for `Data.List.Elem`,
  ...
  used for homogeneous (`===`) and heterogeneous (`~=~`) equality.
...
- Overhaul of the concurrency primitives:
  ...
  - Removed `threadID` and `blodwen-thisthread`. Formerly, in the Chez Scheme
    ...
    value of type `ThreadID`. These are _different kinds of values_ in Chez
